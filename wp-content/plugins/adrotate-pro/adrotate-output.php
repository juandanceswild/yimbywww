...
function adrotate_custom_css() {
	...
	$groups = $wpdb->get_results("SELECT `id`, `modus`, `gridrows`, `gridcolumns`, `adwidth`, `adheight`, `admargin`, `admargin_bottom`, `admargin_left`, `admargin_right` FROM `".$wpdb->prefix."adrotate_groups` WHERE `name` != '' ORDER BY `id` ASC;");
	if($groups) {
		...
		foreach($groups as $group) {
			if($group->modus == 0 AND ($group->admargin > 0 OR $group->admargin_right > 0 OR $group->admargin_bottom > 0 OR $group->admargin_left > 0)) { // Single ad group
				$output_css .= "\t.g-".$group->id." { margin:".$group->admargin."px ".$group->admargin_right."px ".$group->admargin_bottom."px ".$group->admargin_left."px; }\n";
			}
			...
			if($group->modus == 1) { // Dynamic group
				...
				$output_css .= "\t.g-".$group->id." { margin:".$group->admargin."px ".$group->admargin_right."px ".$group->admargin_bottom."px ".$group->admargin_left."px; ".$width." ".$height." }\n";
