	...
	if(wp_verify_nonce($_POST['adrotate_nonce'], 'adrotate_save_ad')) {
		...
		if(current_user_can('adrotate_ad_manage')) {
			...
			if($startdate > 0 AND $enddate > 0) {
				$wpdb->insert($wpdb->prefix.'adrotate_schedule', array('name' => 'Schedule for ad '.$id, 'starttime' => $startdate, 'stoptime' => $enddate, 'maxclicks' => $maxclicks, 'maximpressions' => $maxshown, 'spread' => $spread, 'hourimpressions' => $hourimpressions));
